...
public class WebSocketResource {
  ...
  public void addSession(Session session) {
    ...
    session.getAsyncRemote().sendText(
        session.getId() + "_" + SubscriptionSession.generateAuthHash(session, authString)
    );
  ...
  /**
   * Creates a simple log for all socket throws that are not caught by default operation handlers.
   *
   * @param session the {@link Session} that is currently closing
  ...
  public void handleError(Session session, Throwable th) {
    logger.error(
        "WebSocket error detected. Session ID: {}. Cause: {}.", session.getId(), th.getCause()
    );
  }
